...
void flb_es_list_destroy(struct flb_es_list *ins)
{
    if (ins->value && ins->owned != FLB_FALSE) {
...
void flb_es_list_set_list(struct flb_es_list *dest, struct mk_list *src)
{
...
void flb_es_list_move_list(struct flb_es_list *dest, struct mk_list *src)
{
    flb_es_list_destroy(dest);
    if (src) {
        dest->value = src;
